...
\midrule
\endhead
\PZero & Evaluation truth & A result can become GREEN or resolved only after parser-backed evidence proves that tests actually executed and passed. Empty collection, help, collect-only, no-op runners, stale reports, and unpaired candidates are RED. \\
\PZero & Execution isolation & Headless Bash and test execution require an OS-isolated environment. Docker runs with a verified owner identity and no network by default. Local and worktree environments reject untrusted command execution. Grep paths pass through the workspace jail. \\
...
Split-module regressions & \code{156 passed}; new test modules remain below the 800-line CI ceiling. \\
Repository-wide pytest & \code{2424 passed}; 18 Python 3.14 fork deprecation warnings; 126.02 seconds. \\
...
TeX compilation and page rendering & \Pass; XeLaTeX compiled 13 pages and every rendered page was inspected. \\
\bottomrule
\end{longtable}

\section{Compatibility and Operational Impact}

Headless runs without a Docker image can still read and edit through dedicated filesystem tools, yet Bash and RunTests return an isolation error. A benchmark that needs command execution must provide an isolated Docker environment. Docker starts without network access; tasks that require network setup must prepare dependencies in the image.

Old prediction or report artifacts lacking full candidate identity are no longer accepted as proof for a new attempt. Re-evaluation creates the required sidecars. Native test runners without a parser-backed execution adapter return RED even when their process exits zero. These behaviors protect result truth and can surface previously hidden technical failures.

The rendered PDF is generated for delivery under a local output directory and excluded from the commit. The TeX source is the tracked, reviewable artifact. Public source, comments, commit metadata, and PR text remain English; the requested author name is preserved as a proper name.

\section{Git and Pull Request Plan}

The branch is synchronized with the current upstream \code{main} before publication. The commit and PR title use an English Conventional Commit subject. The push target is the contributor fork, and the draft PR targets \code{YihongDong/OpenCollab:main}. The PR body summarizes root causes, user and evaluator impact, the compatibility changes above, and every executed validation gate.
