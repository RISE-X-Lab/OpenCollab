...
- `env.py` — `LocalEnvironment`, `WorktreeEnvironment`, `DockerEnvironment` (start mode via `setup()`, or attach to a running container via `container_id=`; `exec_workdir`/`command_prefix`/`timeout_returncode` exec hooks). Host-filesystem environments set `local_filesystem = True` (Local/Worktree); remote ones (Docker) leave it false so write tools skip the host write lock
