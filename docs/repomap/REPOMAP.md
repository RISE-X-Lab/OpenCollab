...
- `compaction.py` — `ContextCompactionUseCase` (the *mutating* compaction path). **Retired in the wired runtime**: `build_session_runtime` constructs it but passes `compaction_enabled=False` to `SessionRunUseCase`, so the run loop never routes to `COMPACTING`. No longer exposed on `Session`/`SessionRuntime` (no `compactor` attribute, no `compaction_threshold` knob) nor re-exported from `application/__init__.py`. The class + its unit tests stay intact; the **read-time** path (`AutoCompactShaper`) is now the sole active summarizer.
