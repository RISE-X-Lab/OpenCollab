...
3. **workflow.py(1269 行)专用 runner → 2 个 mixin(搬迁,非合并)**。recon 纠正(task w4xi4dsah):5 runner 只共享**外壳**(build→`_track_session`→try `_run_session_turn`→except-log-None),中段各异(注哪个工具 / `tool_choice` 强制 / harvest 方式)→ **不能塌成一个 helper,只能整体 move**(都只通过 `self.` 碰引擎)。**且只剥 eval 栈到不了 <800**(剩 ~976):structured 强制是**第二个 concern**。故拆**两个单一-concern mixin**:① `application/workflow_agents.py` = eval 防御(`_run_enforced_agent`/`_synthesize_dead_scout`/`draft_findings`/`_draft_findings_with_lease`/`_scout_ledger`/`_emit_*`/`_configure_session_enforcement`,628-920 ~293行);② `application/workflow_structured.py` = structured 强制(`_run_structured_agent`/`_forced_structured_commit`/`_carry_exploration` + `_STRUCTURED_*`/`_named_tool_choice`/`_schema_satisfied`,922-1125+常量 ~240行)。`workflow.py` 只留引擎(`agent`/`parallel`/`pipeline`/budget/lease/timeout/观测/`_run_session_turn`)→ <800。**public 约束**:`WorkflowContext`/`WorkflowBudgetExceeded` 是 SDK 公开、`draft_findings`/`enforcement_strength` 是 live API → compose class 保 `application.workflow` 仍导出全部公开方法。**网**:`test_workflow_context.py` public 面强(够纯 move);动 eval-defense body 前补 harvest/salvage golden-master(现仅 :779 一条 enforcement 测试锁它)。**纯搬无新网**(接线破裂会被抓)。
