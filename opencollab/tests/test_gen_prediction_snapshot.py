...
def test_snapshot_preserves_gitlink_without_copying_submodule_objects(tmp_path):
    ...
    _git(
        ...
        "--add",
        "--cacheinfo",
        f"160000,{submodule_commit},integration",
    )
    _git(
        repo,
        "-c",
    ...
    assert evidence["base_tree"] == base_tree
    assert _git(repo, "ls-tree", "HEAD", "integration").stdout.startswith(
...
def test_snapshot_rejects_inherited_git_object_environment(monkeypatch):
    monkeypatch.setenv("GIT_ALTERNATE_OBJECT_DIRECTORIES", "/hidden/objects")
