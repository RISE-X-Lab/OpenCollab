...
import opencollab.adapters._safe_file_atomic as safe_file_atomic_mod
...
def test_atomic_write_cas_refuses_successor_installed_during_parent_verification(
    ...
    original_verify = safe_file_atomic_mod.verify_atomic_parent_binding
    injected = False
...
def test_atomic_replace_keeps_target_visible_until_backup_is_retired(
    tmp_path,
    monkeypatch,
):
    target = tmp_path / "owner.json"
    ...
    real_retire = atomic_commit_mod.retire_owned_file

    def pause_after_exchange(parent_fd, name, owned_fd, expected_identity, **kwargs):
        if kwargs["path_label"].startswith("previous target for "):
    ...
    monkeypatch.setattr(atomic_commit_mod, "retire_owned_file", pause_after_exchange)

    with ThreadPoolExecutor(max_workers=1) as pool:
        future = pool.submit(safe_files_mod.write_regular_bytes_atomic, target, b"new")
        assert entered.wait(timeout=5)
        try:
            observed = {target.read_bytes() for _attempt in range(100)}
        finally:
            release.set()
...
def test_atomic_recovery_preserves_visible_target_after_tombstone_name_swap(
    tmp_path,
    monkeypatch,
):
    target = tmp_path / "owner.json"
...
def test_atomic_write_reserves_full_failure_recovery_capacity(
    ...
    def exchange_then_fail(first, second, **kwargs):
        nonlocal failed
        result = real_rename_exchange(first, second, **kwargs)
        if not failed and second == target.name:
            failed = True
    ...
    monkeypatch.setattr(atomic_rename_mod, "rename_exchange", exchange_then_fail)
...
def test_existing_target_final_successor_is_never_overwritten(
    ...
    def install_successor_before_commit(first, second, **kwargs):
        nonlocal successor_identity
        if (
            successor_identity is None
            and first.startswith(owned_cleanup_mod.RETIRED_FILE_PREFIX)
            and second == target.name
        ):
            successor = tmp_path / "successor.json"
            successor.write_bytes(b"successor")
            os.replace(successor, target)
        ...
        return real_rename_exchange(first, second, **kwargs)
    ...
    with pytest.raises(OSError, match="identity changed during replace") as raised:
...
def test_existing_target_preserves_successor_installed_after_commit(
    ...
    def track_commit(first, second, **kwargs):
        nonlocal committed
        result = real_rename_exchange(first, second, **kwargs)
    ...
    monkeypatch.setattr(atomic_rename_mod, "rename_exchange", track_commit)
