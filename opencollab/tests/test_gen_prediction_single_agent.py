...
def test_single_agent_cli_accepts_metrics_argument():
    script = Path(__file__).resolve().parents[2] / "swebench" / "gen_prediction.py"
...
def test_extract_patch_fails_closed_when_bounded_diff_reports_overflow(monkeypatch):
    calls = []
    ...
    def fake_docker(*args, **_kwargs):
        ...
        return subprocess.CompletedProcess(
            ...
            86,
            "",
            f"staged patch output exceeded {gp.MAX_EXTRACTED_PATCH_BYTES} bytes",
        )
...
def test_bounded_container_output_stops_continuous_producer_without_disk_buffer():
    command = gp.bounded_container_output_command(
        f"{sys.executable} -c \"import sys; sys.stdout.write('x' * 10000000)\"",
        max_bytes=1024,
        label="test stream",
    )
    ...
    assert b"exceeded its byte limit" in completed.stderr
    assert "mktemp" not in command


def test_bounded_capture_kills_term_ignoring_descendant_after_leader_exit(
    tmp_path,
):
    ...
    assert completed.returncode in {0, 86}
...
def test_bounded_capture_reaps_quiet_descendant_after_leader_exit(tmp_path):
    sentinel = tmp_path / "quiet-leak"
    child_code = (
    ...
    command = gp.bounded_container_output_command(
        producer,
        max_bytes=1024,
        label="quiet leader-exit stream",
    )
