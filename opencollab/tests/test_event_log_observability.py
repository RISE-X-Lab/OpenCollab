...
def test_team_batch_uses_shared_bounded_tail_reader_for_loop_log():
    root = Path(__file__).resolve().parents[2]
    source = (root / "scripts" / "run_team_batch.sh").read_text(encoding="utf-8")

    assert "from swebench_loop_monitor import _load_json, _read_tail_text" in source
    assert "_read_tail_text(pathlib.Path(log_path), 1024 * 1024)" in source
    assert 'pathlib.Path(log_path).read_text' not in source
