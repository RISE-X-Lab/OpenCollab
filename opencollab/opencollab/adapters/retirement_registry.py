...
def _read_records_fd(fd: int) -> list[_RetirementRecord]:
    info = os.fstat(fd)
...
def _encoded_records(records: list[_RetirementRecord]) -> bytes:
...
def _registry_parent(path: str | os.PathLike[str]) -> tuple[str, int, str]:
    absolute = os.path.abspath(path)
    name = os.path.basename(absolute)
    if not name or name in {".", ".."}:
        raise ValueError("internal retirement log path is invalid")
    parent_fd = os.open(
        os.path.dirname(absolute) or os.curdir,
        os.O_RDONLY
        | getattr(os, "O_DIRECTORY", 0)
        | getattr(os, "O_CLOEXEC", 0)
        | getattr(os, "O_NOFOLLOW", 0),
    )
    return absolute, parent_fd, name
...
def _write_registry_payload(
    ...
    try:
        ...
        os.fsync(fd)
        current = os.fstat(fd)
        if not stat.S_ISREG(current.st_mode) or current.st_size != len(payload):
            raise OSError("internal retirement log temporary is invalid")
        return current.st_dev, current.st_ino
    finally:
        os.close(fd)


def _remove_registry_temporary(
    parent_fd: int,
    name: str,
...
def _replace_registry(
    ...
    temporary = f".opencollab-retirement-registry-{uuid.uuid4().hex}.tmp"
    identity: tuple[int, int] | None = None
    try:
        identity = _write_registry_payload(parent_fd, temporary, payload)
        os.replace(
            temporary,
            name,
        ...
        _remove_registry_temporary(parent_fd, temporary, identity)


def _cleanup_registry_temporaries(parent_fd: int) -> None:
    prefix = ".opencollab-retirement-registry-"
    names = [
        name
        for name in os.listdir(parent_fd)
        if name.startswith(prefix) and name.endswith(".tmp")
    ]
    if len(names) > MAX_REGISTRY_TEMPORARIES:
        raise OSError("internal retirement registry has too many stale temporaries")
    removed = False
    flags = os.O_RDONLY | getattr(os, "O_CLOEXEC", 0) | getattr(os, "O_NOFOLLOW", 0)
    for name in names:
        ...
        if (
            ...
            raise OSError("internal retirement registry temporary is unsafe")
        fd = os.open(name, flags, dir_fd=parent_fd)
        try:
            opened = os.fstat(fd)
            current = os.stat(name, dir_fd=parent_fd, follow_symlinks=False)
            ...
            if (
                ...
                raise OSError("internal retirement registry temporary changed")
            os.unlink(name, dir_fd=parent_fd)
            detached = os.fstat(fd)
            if (detached.st_dev, detached.st_ino) != identity or detached.st_nlink != 0:
                raise OSError("internal retirement registry temporary unlink was not owned")
            removed = True
        finally:
            os.close(fd)
    if removed:
...
def _mutate_registry(
    ...
    mutate: Callable[[list[_RetirementRecord]], list[_RetirementRecord]],
) -> None:
    _absolute, parent_fd, name = _registry_parent(path)
    ...
    try:
        ...
        if not stat.S_ISREG(lock_info.st_mode):
            raise OSError("internal retirement log lock is invalid")
        fcntl.flock(lock_fd, fcntl.LOCK_EX)
        locked = True
        _cleanup_registry_temporaries(parent_fd)
        records = _read_records_at(parent_fd, name, missing_ok=True)
        _replace_registry(parent_fd, name, mutate(records))
    finally:
        try:
            if locked:
...
def _record_key(record: _RetirementRecord) -> tuple[int, int, str]:
    return record.parent_dev, record.parent_ino, record.name


def _compact_records(records: list[_RetirementRecord]) -> list[_RetirementRecord]:
    by_entry = {_record_key(record): record for record in records}
    return list(by_entry.values())


def _append_record(path: str, record: _RetirementRecord) -> None:
    def append(records: list[_RetirementRecord]) -> list[_RetirementRecord]:
        by_entry = {_record_key(item): item for item in records}
        by_entry[_record_key(record)] = record
        return list(by_entry.values())

    _mutate_registry(path, append)
...
def register_verified_retirement(parent_fd: int, retired_name: str) -> None:
    ...
    workspace = os.environ.get(INTERNAL_RETIREMENT_WORKSPACE_ENV, "")
    relative = _relative_path(parent_fd, retired_name) if log and workspace else ""
...
def initialize_persistent_retirement_log(path: str | os.PathLike[str]) -> str:
    """Create or validate one bounded cross-process retirement registry."""
    absolute = os.path.abspath(path)
    _mutate_registry(absolute, _compact_records)
    return absolute


def configure_persistent_retirement_log(path: str | os.PathLike[str]) -> str:
    """Select one validated registry for this process and its trusted children."""
    configured = os.environ.get(INTERNAL_RETIREMENT_LOG_ENV, "")
    absolute = initialize_persistent_retirement_log(configured or path)
    if not configured:
        os.environ[INTERNAL_RETIREMENT_LOG_ENV] = absolute
    return absolute
...
def _load_records(path: str | os.PathLike[str]) -> list[_RetirementRecord]:
    _absolute, parent_fd, name = _registry_parent(path)
    try:
        return _read_records_at(parent_fd, name, missing_ok=False)
    finally:
        os.close(parent_fd)
...
def forget_verified_retirements(parent_fd: int, names: tuple[str, ...]) -> None:
    ...
    log = os.environ.get(INTERNAL_RETIREMENT_LOG_ENV, "")
    if log:
        def forget(records: list[_RetirementRecord]) -> list[_RetirementRecord]:
            return [
                record
                for record in records
                if not (
...
def registered_retirement_paths(
    ...
    *,
    ...
    try:
        ...
        with _lock:
            records = list(_records)
        selected_log = persistent_log or os.environ.get(INTERNAL_RETIREMENT_LOG_ENV, "")
        if selected_log:
            try:
                records.extend(_load_records(selected_log))
            except FileNotFoundError:
                pass
